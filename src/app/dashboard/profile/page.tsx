  ...
  return (
    <div className="p-8 pb-32 max-w-2xl">
      <header className="mb-8">
        ...
        <p className="text-zinc-500 dark:text-zinc-400 mt-1">Manage your personal and financial details.</p>
      </header>

      <form onSubmit={handleSave} className="space-y-6">
        {/* Avatar */}
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6">
          <h2 className="text-xs font-semibold text-zinc-900 dark:text-zinc-500 uppercase tracking-wider mb-4">Avatar</h2>
          <div className="flex items-center gap-5">
            <div className="relative group cursor-pointer">
              {avatarUrl ? (
                <img src={avatarUrl} alt="Avatar" className="w-20 h-20 rounded-full object-cover border-2 border-zinc-200 dark:border-zinc-700" />
              ) : (
            ...
            <div>
              <p className="text-zinc-900 dark:text-white font-semibold">{user?.name}</p>
              <p className="text-zinc-900 dark:text-zinc-500 text-sm">{user?.email}</p>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 space-y-4">
          <h2 className="text-xs font-semibold text-zinc-900 dark:text-zinc-500 uppercase tracking-wider flex items-center gap-2">
            <User className="w-3.5 h-3.5" /> Personal Info
          </h2>
          <div>
            <label htmlFor="profile-name" className="block text-sm font-medium text-zinc-900 dark:text-zinc-300 mb-1.5">Full Name</label>
            <input
              id="profile-name"
              type="text"
              value={effectiveName}
          ...
          <div>
            <label className="block text-sm font-medium text-zinc-900 dark:text-zinc-300 mb-1.5">Email</label>
            <input
              type="email"
              value={user?.email ?? ""}
              disabled
              className="w-full rounded-lg border border-zinc-200 dark:border-zinc-700/50 bg-zinc-100 dark:bg-zinc-800/20 px-4 py-3 text-zinc-900 dark:text-zinc-500 outline-none text-sm cursor-not-allowed"
            />
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl p-6 space-y-4">
          ...
          <div>
            <label htmlFor="occupation" className="block text-sm font-medium text-zinc-900 dark:text-zinc-300 mb-1.5 flex items-center gap-1.5">
          ...
          <div>
            <label htmlFor="currency" className="block text-sm font-medium text-zinc-900 dark:text-zinc-300 mb-1.5 flex items-center gap-1.5">
          ...
          {/* AI Persona Setting */}
          <div>
            <label htmlFor="ai-persona" className="block text-sm font-medium text-zinc-900 dark:text-zinc-300 mb-1.5 flex items-center gap-1.5">
            ...
            <p className="text-xs text-zinc-900 dark:text-zinc-500 mt-1.5">Changes how the AI speaks to you in the Goal Coach.</p>
          </div>

          <div>
            <label htmlFor="monthly-income" className="block text-sm font-medium text-zinc-900 dark:text-zinc-300 mb-1.5">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label htmlFor="monthly-budget" className="text-sm font-medium text-zinc-900 dark:text-zinc-300">
              ...
              <div className="mt-3 bg-indigo-950/30 border border-indigo-500/30 rounded-xl p-4 space-y-3">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 bg-indigo-600 hover:bg-indigo-500 text-zinc-900 dark:text-white text-xs px-3 py-2 rounded-lg transition-colors shrink-0"
                  >
                    <CheckCircle2 className="w-3.5 h-3.5" />
                    Apply
                  </button>
                </div>
                <p className="text-xs text-zinc-500 dark:text-zinc-400 leading-relaxed">{budgetSuggestion.reasoning}</p>
                {budgetSuggestion.breakdown && Object.keys(budgetSuggestion.breakdown).length > 0 && (
                  <div>
                    <p className="text-xs text-zinc-900 dark:text-zinc-500 mb-2 uppercase tracking-wider">Suggested Breakdown</p>
                    <div className="grid grid-cols-2 gap-1.5">
                      {Object.entries(budgetSuggestion.breakdown).map(([cat, amt]) => (
                        <div key={cat} className="flex justify-between text-xs bg-white dark:bg-zinc-900/60 rounded px-2 py-1.5">
        ...
        <Button
          type="submit"
          disabled={isSaving}
          className="w-full bg-indigo-600 hover:bg-indigo-500 text-zinc-900 dark:text-white py-3 rounded-lg font-semibold transition-all flex items-center justify-center gap-2"
        >
