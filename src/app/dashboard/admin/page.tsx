  ...
  return (
    <div className="p-8 pb-32 max-w-[1400px] mx-auto">
      <header className="mb-8">
        ...
        <p className="text-zinc-400 mt-1">Platform-wide overview, access controls, and system settings.</p>
      </header>
      ...
      <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h3 className="text-sm font-medium text-zinc-400">Total Registered Users</h3>
                </div>
                {isStatsLoading ? (
                  <div className="h-9 w-20 bg-zinc-800 animate-pulse rounded mt-1" />
                ) : (
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
                ...
                {isStatsLoading ? (
                  ...
                  <p className="text-3xl font-bold text-white">{stats?.totalReceipts?.toLocaleString() || 0}</p>
                )}
              </div>
              <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
                <div className="flex items-center gap-2 mb-2">
                  <Activity className="w-4 h-4 text-indigo-400" />
                  <h3 className="text-sm font-medium text-zinc-400">Total Active Goals</h3>
                </div>
                {isStatsLoading ? (
                  <div className="h-9 w-20 bg-zinc-800 animate-pulse rounded mt-1" />
                ) : (
                  <p className="text-3xl font-bold text-white">{stats?.totalGoals?.toLocaleString() || 0}</p>
                )}
              </div>
            </div>

            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
              <h3 className="text-base font-semibold text-white mb-6">User Growth (Last 30 Days)</h3>
        ...
        {activeTab === "users" && (
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden flex flex-col">
            <div className="p-5 border-b border-zinc-800 flex flex-col sm:flex-row gap-4 justify-between items-center bg-zinc-950/50">
              ...
              <div className="relative w-full sm:w-72">
                ...
                <input 
                  ...
                  className="w-full bg-zinc-900 border border-zinc-700 text-sm text-white rounded-lg pl-9 pr-4 py-2 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
                />
              </div>
            </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm text-zinc-400">
                ...
                <tbody className="divide-y divide-zinc-800/50">
                  {isUsersLoading ? (
                    [...Array(5)].map((_, i) => (
                      <tr key={i}><td colSpan={4} className="px-6 py-4"><div className="h-6 bg-zinc-800 animate-pulse rounded" /></td></tr>
                    ))
                  ) : paginatedUsers.length > 0 ? (
                    paginatedUsers.map(user => (
                      <tr key={user.id} className="hover:bg-zinc-800/30 transition-colors">
                        <td className="px-6 py-4">
                          <div className="font-medium text-zinc-200">{user.name || "Unnamed"}</div>
              ...
              <div className="p-4 border-t border-zinc-800 flex items-center justify-between bg-zinc-950/30">
                ...
                <div className="flex items-center gap-2">
                  <button 
                    ...
                    className="p-1.5 rounded bg-zinc-800 text-zinc-400 hover:bg-zinc-700 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed"
                  >
        ...
        {activeTab === "reports" && (
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 text-center py-16">
            <div className="w-16 h-16 bg-indigo-500/10 rounded-full flex items-center justify-center mx-auto mb-4 border border-indigo-500/20">
              <Download className="w-8 h-8 text-indigo-400" />
            </div>
            <h3 className="text-xl font-bold text-white mb-2">Platform Data Extraction</h3>
            ...
            <button 
              onClick={exportToCSV}
              className="px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-medium rounded-lg transition-colors flex items-center gap-2 mx-auto shadow-lg shadow-indigo-500/20"
            >
        ...
        {activeTab === "settings" && (
          <div className="space-y-6">
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
              <h3 className="text-base font-semibold text-white mb-6 border-b border-zinc-800 pb-4">Platform Configuration</h3>
              
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-sm font-medium text-zinc-200">Allow New Registrations</h4>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="text-sm font-medium text-zinc-200">Maintenance Mode</h4>
            ...
            <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6">
              <h3 className="text-base font-semibold text-white mb-2">Global System Prompt</h3>
              <p className="text-xs text-zinc-400 mb-4">Modify the core personality and instructions for the Gemini Agentic Chat across the entire platform.</p>
              
              <textarea
                value={settings.systemPrompt}
                onChange={(e) => setSettings(s => ({ ...s, systemPrompt: e.target.value }))}
                className="w-full h-32 bg-zinc-950 border border-zinc-800 rounded-lg p-4 text-sm text-zinc-300 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 custom-scrollbar mb-4"
              />
